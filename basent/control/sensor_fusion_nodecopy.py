...
class SensorFusionNode(Node):
    def __init__(self, debug_csv=True):
        ...
        self.ekf.x[6:10] = [0.0, 0.0, 0.0, 1.0]  # identity quaternion
        self.ekf.P *= 0.1
    ...
    def state_transition(self, x, dt, imu_accel=None):
        """
        EKF prediction step using IMU acceleration.
        State: [px, py, pz, vx, vy, vz, qx, qy, qz, qw]
        imu_accel: acceleration in body frame (ax, ay, az)
        """
        new_x = x.copy()
        px, py, pz = x[0:3]
        vx, vy, vz = x[3:6]
        ...
        # Update velocity using IMU acceleration
        if imu_accel is not None:
            # Convert body accel to world frame
            R_world_body = R.from_quat([qx, qy, qz, qw]).as_matrix()
            a_world = R_world_body @ imu_accel  # 3x1
        ...
        new_x[0:3] = [px, py, pz]
        new_x[3:6] = [vx, vy, vz]
        new_x[6:10] = [qx, qy, qz, qw]  # Orientation unchanged
        return new_x
    ...
    def state_transition_jacobian(self, dt):
        ...
        # ∂v/∂q is neglected here (small dt approximation)
        # ∂v/∂v already 1 (identity)
        # ∂p/∂p already 1 (identity)
        # ∂q/∂q already 1 (identity)
    ...
    def update_ekf(self):
        ...
        if self.imu_data is not None:
            imu_accel_vector = np.array([
                self.imu_data.linear_acceleration.x,
                self.imu_data.linear_acceleration.y,
                self.imu_data.linear_acceleration.z
            ])
        else:
            imu_accel_vector = None

        # State prediction with IMU acceleration
        self.ekf.x = self.state_transition(self.ekf.x, dt, imu_accel=imu_accel_vector)
        ...
        # Orientation for projecting sensors
        q = self.ekf.x[6:10]
        ...
        def project_to_world_z(measurement, sensor_axis=np.array([0, 0, 1])):
            ...
            return measurement * world_vector[2]

        laser = project_to_world_z(laser_raw)
        ultra = project_to_world_z(ultra_raw)
        ...
        altitudes = []
        weights = []
        self.get_logger().info(f'Altitudes - Baro: {baro}, Alt: {alt}, Laser: {laser}, Ultra: {ultra}, GPS: {gps_alt}')
        self.altitude_log_file.flush()
        if baro is not None:
        ...
        self.get_logger().info(f'Pose: {z[0:3]}, Altitude: {fused_alt}')
        # --- EKF UPDATE ---
        if valid_measurement:
            ...
            qx, qy, qz, qw = self.ekf.x[6:10]
            norm = np.linalg.norm([qx, qy, qz, qw])
            if norm > 1e-8:
                self.ekf.x[6:10] = [qx, qy, qz, qw] / norm
            else:
                self.ekf.x[6:10] = [0.0, 0.0, 0.0, 1.0]
